...
- **Length:** chapters are **full-size novel chapters**: as substantial as a chapter in a printed novel, as long as the scene work demands, no fixed word target, no padding and no compressing (a three-page chapter is not a novel chapter). Typing-session boundedness is handled by per-chapter typing-progress persistence (resume with a one-paragraph rewind), not by keeping chapters short. (Superseded an earlier 400-900-word target; noted in `docs/DECISIONS.md`.)
