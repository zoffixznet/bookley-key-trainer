...
- **Auth gating (must be robust — the user called this out explicitly):** the app must never hang or silently fail when `claude` is not authenticated. Handle it on multiple layers:
  - **Before** any book run, probe login state (e.g. `claude auth status`, or a trivial `claude -p` probe) and, if not logged in, do not attempt generation. Show a clear **in-app sign-in prompt**: explain that Claude Code is not signed in, show the exact command to run (`claude auth login`, or `claude setup-token` for a long-lived token), make it copyable, and offer a **"Re-check" button** that re-probes and proceeds once the user has logged in out-of-band.
  - **Never attempt an interactive login inside a `-p` run** (interactive `/login` is unavailable in headless mode). Always run the child with a **timeout / cancel path** so a misbehaving or blocked CLI can never freeze the UI thread; run it off the UI thread and stream results back.
  - **During** a run, treat an `authentication_failed` (or `oauth_org_not_allowed`) `api_retry` category, or a non-zero exit that indicates auth failure, the same way: stop, preserve any prior chapters, and surface the same sign-in prompt with a retry.
...
8. **Resilience — predictable failures degrade, never crash the process:**
   - `claude` missing / logged out / erroring / rate-limited / max-turns → clear in-app state, actionable guidance (e.g. an in-app sign-in prompt showing `claude auth login` with a re-check button), prior chapters preserved, retry possible; the app **never hangs** waiting on the CLI (run it off-thread with a timeout/cancel), and the other three content modes keep working regardless, even with no `claude` installed at all.
...
- [ ] The README covers install, configuration, running, and **Known limitations** (Wayland untested here; live book generation needs `claude` login and consumes subscription usage; the Anthropic ToS note on distributing subscription-driven apps), all built around the make targets.
